...
def job_config = [
        job: [
                name: "DemoTest1_Pipeline_develop"
        ],
        git: [
                branch: "develop"
        ]
...
def customLib = library identifier: 'DemoTest1_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/DemoTest1_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = lib.de.allianz.DemoTest1.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                    jslGit.checkout( config, "JEQP", "DemoTest1_Pipeline_develop", "develop")
                }
            }    
        }
        stage('Build') {
